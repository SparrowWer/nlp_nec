...
# CUDA_VISIBLE_DEVICES="2,3" python qwenchat.py
device = "cuda" # 加载模型的设备
# model_name_or_path="/data0/zhangchongrui/model/compused"
...
messages = [
    ...
    {"role": "user", "content": "您将扮演一个严谨且忠实于所提供的背景信息的客服助手。您的任务是根据提供的背景信息回答用户的问题。请仔细阅读并记住下面的背景信息，然后根据这些背景信息，\
...
for prompt in [msg["content"] for msg in messages if msg["role"] == "user"]:
